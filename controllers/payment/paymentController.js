...
class paymentController {
    create_stripe_connect_account = async (req, res) => {
        ...
        try {
            const stripInfo = await stripModel.findOne({ sellerId: id })

            if (stripInfo) {
                await stripModel.deleteOne({ sellerId: id })
    ...
    active_stripe_connect_account = async (req, res) => {
        ...
        const { id } = req
        try {
            const userStripeInfo = await stripModel.findOne({ code: activeCode })
            if (userStripeInfo) {
    ...
    payment_request_confirm = async (req, res) => {
        ...
        try {
            ...
            const { stripeId } = await stripModel.findOne({
